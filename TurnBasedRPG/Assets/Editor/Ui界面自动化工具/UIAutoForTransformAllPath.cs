...
public class UIAutoForTransformAllPath : EditorWindow
{
    ...
    {
        GetWindow(typeof(UIAutoForTransformAllPath), false, "Transform组件查找").Show();
    }
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.LabelField("Transform组件全路径查找", EditorStyles.boldLabel);
            ...
            {
                GetComonpentProperty(new FindConfig() { isGetSet = true, KeyValue= TransformPrefix ,});
            }
            if (GUILayout.Button("获取变量", EditorStyles.miniButtonMid))
            {
                GetComonpentProperty(new FindConfig() {isGetSet=false, KeyValue = TransformPrefix, });
            }
            //******************************获取组件******************************
            ...
            {
                GetComonpentFind(new FindConfig()
                {
                    isAssign = true,
                });
            }
    ...
    /// 获取监听
    /// </summary>
    private void GetComonpentListener()
    {
        //获取到当前选择的物体
        GameObject obj = Selection.objects.First() as GameObject;
    ...
    /// 一键获取
    /// </summary>
    private void GetAll(FindConfig findtConfig)
    {
        //获取到当前选择的物体
        GameObject obj = Selection.objects.First() as GameObject;
    ...
    /// 直接获取组件--生成组件获取代码
    /// </summary>
    private void GetComonpentFind(FindConfig findtConfig)
    {
        StringBuilder sb = new StringBuilder();
        GameObject obj = Selection.objects.First() as GameObject;
        findtConfig.selectGoName = obj.name;
    ...
    private string GetTransformPath(Transform transformTF, string selectGoName)
    {
        //临时变量-存放路径
    ...
    /// <returns></returns>
    private string ComonpentProperty(FindConfig findtConfig)
    {
    ...
    /// <returns></returns>
    private string ComonpentFind(FindConfig findtConfig)
    {
        if (findtConfig.controlDic.Count == 0) return null;
        StringBuilder sb = new StringBuilder();
    ...
    /// <param name="controlDic">物体的字典</param>
    /// <returns></returns>
    private string ComonpentListener(Dictionary<string, List<Component>> controlDic)
    {
    ...
    /// <returns></returns>
    private string TypeChange(string typeStr)
    {
        switch (typeStr)
        {
    ...
    /// 字符串去除特殊符号空白等
    /// </summary>
    public string ClearSpecificSymbol(string str)
    {
